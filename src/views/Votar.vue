<template>
  <main v-if="pizzeria" id="main" class="pizzeria">
    <template v-if="cargado && !yaVotaste">
      ...
      <p v-if="faltan == 1" id="faltan">
        <span
          >Falta <big>{{ faltan }}</big> jurado en la sala...</span
        >
      </p>
      <p v-if="faltan > 1" id="faltan">
        <span
          >Faltan <big>{{ faltan }}</big> jurados en la sala...</span
        >
      </p>
    </template>
      ...
      <div id="votos">
        ...
        <ul>
          <li v-for="(voto, index) in votos" :key="index" :class="{ mio: voto.mio }">
            ...
            <span v-if="voto.secreto">{{ voto.voto ? `${voto.voto} (secreto)` : '?' }}</span>
            <span v-if="!voto.secreto">{{ voto.voto ? voto.voto : '-' }}</span>
          </li>
        </ul>
      </div>
...
export default {
  ...
  computed: {
    yaVotaste() {
      if (!this.challenge.abierto) return true;
  ...
  methods: {
    load() {
      ...
      api.get(`challenges/${this.challengeId}`).then(async data => {
        ...
        this.challenge = data;
        this.pizzeria = data.pizzerias
          .filter(p => {
            return p.id == this.pizzeriaId;
          })
          .pop();
    ...
    setActiveVote() {
      document.querySelectorAll('.selected').forEach(slide => {
    ...
    votar() {
      if (this.yaVotaste || this.votando) return false;
    ...
    async actualizarVotos() {
      return api.get(`pizzerias/${this.pizzeriaId}/votos`).then(data => {
        ...
        const publicos = this.votos.filter(voto => {
          return voto.voto;
        });
        ...
        if (total > 0) this.promedio = (total / publicos.length).toFixed(2);
      });
    },
  },
};
