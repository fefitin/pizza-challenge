...
module.exports = function(firebase) {
  ...
  app.use('/api/*', (req, res, next) => {
    ...
    res.append('Access-Control-Allow-Origin', 'http://192.168.0.2:8080');
  ...
  app.get('/api/pizzerias/:id/votos', checkLogin, async (req, res) => {
    ...
    const ref = firebase
      .firestore()
      .collection('votos')
      .where('pizzeriaId', '=', `pizzerias/${req.params.id}`);

    try {
      const pDoc = await pRef.get();
      const pizzeria = pDoc.data();

      const doc = await ref.get();
      const data = {
        ok: true,
        votos: doc.docs.map(d => {
          return d.data();
        }),
      };

      if (!pizzeria.secretosVisibles) {
